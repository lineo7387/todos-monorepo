...
import { deriveMobileTaskView } from "./task-view.ts";
...
describe("deriveMobileTaskView", () => {
  test("excludes undated tasks from due-today and upcoming date views", () => {
    const result = deriveMobileTaskView(todos, "all", "upcoming", "2026-04-21", "2026-04-21");
  ...
  test("keeps selected-day inspection scoped to the current task filter", () => {
    const result = deriveMobileTaskView(todos, "completed", "all", "2026-04-21", "2026-04-24");
