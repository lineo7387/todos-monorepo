...
The system SHALL keep workspace selection, task creation, editing, completion, deletion, refresh behavior, team join, and lightweight task organization functionally consistent across supported clients, even if layouts differ by platform.
...
- **WHEN** an authenticated user selects or navigates to a different workspace on the React Native client
